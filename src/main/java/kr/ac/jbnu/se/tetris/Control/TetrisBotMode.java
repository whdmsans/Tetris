...
public class TetrisBotMode extends JPanel {
    ...
    public TetrisBotMode(Tetris parent) {
        statusbar = TestMonitor.getScoreBar(false);//P1일 경우 false반환임. 이를 수동으로 고정하였으므로 추후 변경 요망.
...
//                    TIME =  2;
                    ...
                    for (int j = 0; j < curPiece.getNumOfRotate(); j++) {
                        ...
                        if (big_weight < (double) ret[0]) {
                            ...
                            //goodPosition.copyShape((Shape) ret[1]); origin code
...
//                    t_board.repaint();
                ...
                clearBoard();
                //chromosome.fitness += current_game_score; origin code
...
//    }
    ...
    //private Object[] move(Shape curPiece, Chromosome chromosome) { origin code
    private Object[] move(Entity curPiece, Chromosome chromosome) {
        ...
        //block.copyShape(curPiece); origin code
        block = new Entity(curPiece.getShape());
        move_left(block);
        //Shape temp_block = new Shape(); origin code
        ...
        while (true) {
            ...
            if (end_down = dropDown(temp_block)) {
                ...
                //double fitness = calculator.blockFitness(chromosome.weight); origin code
                double fitness = calculator.blockFitness(chromosome.getWeight());
                if (big_weight < fitness) {
                    big_weight = fitness;
                    //big_weight_block.copyShape(temp_block); origin code
    ...
    //private void deleteBlock(Shape shape) { origin code
    ...
    //private void placeShape(Shape shape) { origin code
    private void placeShape(Entity shape) {
        ...
        for(int i = 0; i < 4; i++) {
            ...
            //board[idx] = shape.getBlockNum(); origin code
            board[idx] = shape.getBlockNum();
        }
    }

    //private boolean move_right(Shape shape) { origin code
    private boolean move_right(Entity shape) {
        return !tryMove(shape, shape.position[0] + 1 , shape.position[1]);
    }

    //private void move_left(Shape shape) { origin code
    ...
    //private boolean finish(Shape shape) { origin code
...
//        }
    ...
    //private Boolean dropDown(Shape curPiece) { origin code
    private Boolean dropDown(Entity curPiece) {
    ...
    //private boolean tryMove(Entity newPiece, int newX, int newY) { origin code
    private boolean tryMove(Entity newPiece, int newX, int newY) {
        for (int i = 0; i < 4; ++i) {
...
//        curPiece = newPiece;
        //newPiece.setPosition(newX, newY); origin code
